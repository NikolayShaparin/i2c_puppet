	...
	if (p_item->effective_key == '\0') {
		char key = p_entry->chr;
			switch (p_entry->mod) {
				...
				case KEY_MOD_ID_ALT:
					// Do not propagate ALT presses from arbitrary ALT-mod keys to the host.
					// Only the physical `KEY_BTN_LEFT1` should send a Left-Alt to the host.
					if (p_entry->chr == KEY_BTN_LEFT1) {
						key = KEY_MOD_ALT;
					} else {
						key = '\0';
					}
			...
			default:
			{
				if (reg_is_bit_set(REG_ID_CFG, CFG_USE_MODS)) {
					...
					const bool alt = self.mods[KEY_MOD_ID_SYM] || self.mods[KEY_MOD_ID_ALT] || self.numlock;
